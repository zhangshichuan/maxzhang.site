...
你已经看了十分钟了。眼睛开始发酸。屏幕上每一行都他妈长得差不多。你知道出错了——不然运维不会半夜叫你——但你完全看不出错在哪里。
...
你关掉电脑。躺在黑暗里。脑子里只有一个念头：操，如果日志写得够好，这条信息应该在五秒内出现在我眼前。不是十分钟。
...
少打了几十个字符，还不用对齐冒号。这种小东西用习惯了以后，再看老代码里那一串串的字符串拼接，你会有一种"我他妈以前在干什么"的感慨。
...
你这个周末搭了个小项目。一台服务器，日志用字符串打印，看着挺舒服：
...
// 你调 log 的时候如果没传参数，log 函数自己抛个异常把调用者炸了。这他妈是帮倒忙。
```
