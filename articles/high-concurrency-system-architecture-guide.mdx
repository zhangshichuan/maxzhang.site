...
class RedisCache {
	...
	async acquireLock(lockKey, ttl) {
		const result = await this.setAsync(lockKey, 'locked', 'NX', 'EX', ttl)
		return result === 'OK'
	}
	...
	async tag(tag, keys) {
		...
		await this.setAsync(tagKey, JSON.stringify(keys))
	...
	async getByTag(tag) {
		const tagKey = `tag:${tag}`
		const data = await this.getAsync(tagKey)
...
async function getWithBackgroundRefresh(key, ttl, fetchFunction) {
	...
	// 检查是否需要刷新
	const ttlRemaining = await cache.ttlAsync(key)
...
```mermaid
graph TD
    A[应用层] --> B[数据库中间件<br/>ProxySQL/MyCat]
    B --> C[主库<br/>处理写请求]
    B --> D[从库1<br/>处理读请求]
    B --> E[从库2<br/>处理读请求]
    B --> F[从库3<br/>处理读请求]
...
mysql_servers:
(
    { address="10.0.0.1", port=3306, hostgroup=0, weight=100 },  # 主库组
    { address="10.0.0.2", port=3306, hostgroup=1, weight=100 },  # 从库组
    { address="10.0.0.3", port=3306, hostgroup=1, weight=100 },
    { address="10.0.0.4", port=3306, hostgroup=1, weight=100 }
)

mysql_query_rules:
(
    # 写请求路由到主库
    { rule_id=1, active=1, match_pattern="^SELECT.*FOR UPDATE", destination_hostgroup=0 },
    { rule_id=2, active=1, match_pattern="^(INSERT|UPDATE|DELETE)", destination_hostgroup=0 },

    # 读请求路由到从库
    { rule_id=3, active=1, match_pattern="^SELECT", destination_hostgroup=1 },

    # 默认路由到主库
    { rule_id=4, active=1, match_pattern=".*", destination_hostgroup=0 }
)
```
...
BEGIN
    ...
    SET table_index = p_user_id % 4;

    CASE table_index
        WHEN 0 THEN INSERT INTO user_0 VALUES (p_user_id, p_username, p_email);
        WHEN 1 THEN INSERT INTO user_1 VALUES (p_user_id, p_username, p_email);
        WHEN 2 THEN INSERT INTO user_2 VALUES (p_user_id, p_username, p_email);
        WHEN 3 THEN INSERT INTO user_3 VALUES (p_user_id, p_username, p_email);
    END CASE;
END$$
DELIMITER ;
```
...
class ShardingManager {
     ...
     * @param {string} sql 原始SQL（使用逻辑表名 t_order）
     */
    async executeQuery(userId, orderId, sql, params) {
        ...
        // 将逻辑表名替换为实际表名
        const finalSql = sql.replace('t_order', tableName);

        console.log(`路由至数据库节点: ${userId % this.dbClusters.length}, 表名: ${tableName}`);
...
const pool = mysql.createPool({
	...
	keepAliveInitialDelay: 10000, // 保持连接延迟
	idleTimeout: 60000, // 空闲超时
	maxIdle: 10, // 最大空闲连接
	maxUpsertBatchSize: 1000, // 批量操作大小
})
...
async function queryWithRetry(sql, params, retries = 3) {
	for (let i = 0; i < retries; i++) {
		try {
			...
			console.error(`查询失败，重试 ${i + 1}/${retries}:`, err)
