...
LVS (Linux Virtual Server) forwards traffic at the transport layer (TCP/UDP). It has extremely high performance and is often the first checkpoint for incoming traffic. LVS has three working modes. Here is an example of NAT mode:
...
```mermaid
flowchart TD
    A[Data Collection - Prometheus Exporters] --> B[Aggregation - Prometheus]
    B --> C[Visualization - Grafana]
    B --> D[Alerting - AlertManager]
    D --> E[Notifications - Slack/Email]
```
