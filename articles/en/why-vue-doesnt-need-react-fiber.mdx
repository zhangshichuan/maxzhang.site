...
**React's Virtual DOM is "brute force comparison."** When state changes, React creates a new virtual DOM tree and compares it completely with the old tree (Diff). This comparison's time complexity is O(n³), but React reduces it to O(n) through assumptions ("two elements of different types will produce different trees").
