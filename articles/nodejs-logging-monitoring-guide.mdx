...
在 Node.js 中，`console.log` 实际上是输出到标准输出流（stdout），而 `console.error` 输出到标准错误流（stderr）。这两个流在底层实现上存在差异，尤其是在不同操作系统和终端环境下：

- **TTY（终端）**：当输出到交互式终端时，`stdout` 和 `stderr` 通常是同步的，但 `stdout` 可能会被缓冲。
- **管道/重定向**：当将输出重定向到文件或管道时，`stdout` 是同步的（Node.js 会等待数据写入完成），但性能较低；而 `stderr` 在管道中通常是同步的。

> **注意**：在 Windows 和 Linux 上，流的行为可能略有不同。因此，在生产环境中，直接使用 `console.log` 写入日志文件并不是最佳选择，因为它无法提供日志切割、级别过滤等高级功能。
...
在 Node.js 中，我们可以使用 `winston` 或 `bunyan` 将日志输出为 JSON 格式，并写入文件。Filebeat 会读取该文件并将日志传输到 Logstash。
